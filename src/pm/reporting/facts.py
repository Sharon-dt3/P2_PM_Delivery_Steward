...
one ProjectSnapshot (PM-05, extended by this task to also carry sprints/
commitments/risks) and nothing else -- no adapter call, no clock read, no
randomness. That purity is what the row's own acceptance test asks for
literally: "Regenerating on the same snapshot yields identical facts;
wording may differ." Calling this function twice on the same snapshot
object always returns two equal MorningBriefFacts instances (see
tests/unit/test_reporting_facts.py); only pm.reporting.morning_brief's
own model call downstream of these facts is allowed to vary.
